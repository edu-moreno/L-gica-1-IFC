...
def dias_para_segundos(dias, horas, minutos, segundos):
    ...
    minutos_para_segundo = minutos * 60
    segundos_para_segundo = segundos * 1
    soma = dias_para_segundos + horas_para_segundos + minutos_para_segundo + segundos_para_segundo
    return round(soma, 2)
    return soma
...
def dias_perdidos_por_fumar(cigarros_fumados_por_dia, anos_fumando):
    ...
    quantidades = 1
    minutos = 10
    dias_perdidos_pro_cachimbo_da_paz = (((cigarros_fumados_por_dia * anos_fumando)* 365) * 10) / 1440
    return round(dias_perdidos_pro_cachimbo_da_paz, 2)
...
def media_final_aprovado_reprovado(p1, p2, ep1, ep2):
    ...
    media = p1 * (0.70 + p2 * 0.70) + (ep1 * 0.30 + ep2 * 0.30)
    pau = media / 4
    return pau > 7
...
def duzias(ovos):
    ...
    quantidade_de_duzias = ovos / 12
    return round(quantidade_de_duzias)
